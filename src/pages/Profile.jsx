...
export default function UserProfile() {
  ...
  const handleUploadAvatar = async (e) => {
    ...
    try {
      const res = await fetch(`${backendUrl}/users/upload-avatar`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
  ...
  const handleCancelSubscription = async () => {
    try {
      const res = await fetch(`${backendUrl}/stripe/cancel`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    ...
    if (params.get("success") === "true") {
      fetch(`${backendUrl}/stripe/activate`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
          ...
          {user ? (
            <div className="flex flex-col items-center w-full">
              <div className="relative mb-8 group">
                ...
                <input onChange={handleUploadAvatar} type="file" id="avatarInput" className="hidden" />
              </div>
                ...
                {!editMode ? (
                  <div className="flex flex-col gap-3 w-[30%] text-center">
                    ...
                    {user?.subscriptionActive ? (
                      <button
                        ...
                        className="px-6 py-2 bg-red-700 hover:bg-red-600 rounded-xl shadow-md hover:shadow-red-600/40 text-white transition"
                      >
                        Cancel Subscription
                      </button>
                    ) : (
                  ...
                  >
                    ...
                    <div className="flex gap-3 mt-3">
                      <button type="submit" className="flex-1 bg-green-700 hover:bg-green-600 text-white py-2 rounded-md shadow-md hover:shadow-green-700/40">
                        Save
                      </button>
                      <button
                        type="button"
